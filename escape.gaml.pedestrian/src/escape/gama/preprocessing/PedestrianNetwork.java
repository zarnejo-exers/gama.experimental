...
public class PedestrianNetwork {
	...
	public static IList<IShape> generateNetwork(final IScope scope, final IList<String> obstaclesPath,
			final String boundsPath, final double valTolClip, final double valTolTri, final double valFiltering,
			final boolean openAreaManagement, final double valDistForOpenArea, final double valDensityOpenArea,
			final boolean randomDist, final boolean cleanNetwork) {
		...
		if (boundsShape == null) {
			for (GamaShapeFile shp : obstacleShapefiles) {
				if (bounds == null) {
					bounds = shp.computeEnvelope(scope).toGeometry();
				} else {
					bounds = bounds.union(shp.computeEnvelope(scope).toGeometry());
				}
			}
		} else {
	...
	public static IShape managementOpenArea(final IScope scope, IShape area, final double valDistForOpenArea,
			final double valDensityOpenArea, final boolean randomDist) {
		...
		if (areaTmp != null) {
			...
			for (IShape g : areaTmp.getGeometries()) {
				...
				if (randomDist) {
					...
					for (IShape sq : squares) {
						pts.add(sq.getCentroid());
					}
				}
			}
