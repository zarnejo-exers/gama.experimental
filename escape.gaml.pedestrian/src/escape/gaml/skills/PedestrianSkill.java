...
public class PedestrianSkill extends MovingSkill {
	...
	public double walkWithForceModel(final IScope scope, final IAgent agent, final GamaPoint currentTarget,
			final IShape bounds, final IContainer obstaclesList) {
		...
		if (!(Random.opFlip(scope, 1.0 - proba_detour) && location.euclidianDistanceTo(currentTarget) > agent
				.getLocation().euclidianDistanceTo(currentTarget))) {
		 ...
		 *
		 * cpt++; if (cpt % 1000 == 0) { System.out.println("t1: " + t1 + " t2: " + t2+ " t3: " + t3+ " t4: " + t4+
		 * " t5: " + t5+ " t6: " + t6+ " t7: " + t7+ " t8: " + t8+ " t9: " + t9+ " t10: " + t10); }
		 */
		return realSpeed;
	}

	public GamaPoint avoid(final IScope scope, final IAgent agent, final GamaPoint location, final GamaPoint velocity,
			final GamaPoint currentTarget, double maxDist, final IContainer obstaclesList) {
		// long t = System.currentTimeMillis();
		...
		// for (ISpecies species : speciesList) {
		obstacles.addAll(Spatial.Queries.at_distance(scope, obstaclesList, maxDist));
		// }
		...
		for (IAgent a : obstacles) {
			...
			double w = lambda == 1.0 ? 0.0
					: lambda + (1 - lambda) * (1 - Maths.cos(
							Spatial.Punctal.angleInDegreesBetween(scope, location, a.getCentroid(), currentTarget)))
							/ 2.0;
			GamaPoint locA = a.getGeometry().isPoint() ? a.getLocation().copy(scope)
					: Spatial.Punctal.closest_points_with(location, a.getGeometry()).get(1);
			GamaPoint vector = locA.minus(location);
			...
			if (g > 0) {
				...
				delta = (right ? new GamaPoint(-vector.y, vector.x) : new GamaPoint(vector.y, -vector.x))
						.multiplyBy(k / distance);
			}
