...
public class PedestrianEscapeSkill extends PedestrianSkill {
	...
	@setter (VELOCITY)
	public void setVelocity(final IAgent agent, final GamaPoint point) {
	...
	public IPath primComputeVirtualPath(final IScope scope) throws GamaRuntimeException {
		...
		final GamaSpatialGraph graph = (GamaSpatialGraph) scope.getArg(PEDESTRIAN_GRAPH, IType.GRAPH);
		...
		int index = 0;
		for (GamaPoint pt : thePath.getGeometry().getPoints()) {
			targets.add(pt);
		}
		if (!targets.lastValue(scope).equals(target)) {
		...
		for (GamaPoint tar : targets) {
			...
			if (notOk) {
				if (tar == targets.firstValue(scope)) {
					roadTarget.put(tar, edges.lastValue(scope));
				} else {
					roadTarget.put(tar, edges.firstValue(scope));
				}
			}
	...
	public void primWalkEscape(final IScope scope) throws GamaRuntimeException {
		...
		while (movement) {
			...
			if (arrivedAtTarget(scope, location, currentTarget, getShoulderLength(agent) * 2, index, maxIndex,
					targets)) {
	...
	boolean arrivedAtTarget(final IScope scope, final GamaPoint location, final GamaPoint currentTarget,
			final double size, final int index, final int maxIndex, final IList<GamaPoint> targets) {
		double dist = location.euclidianDistanceTo(currentTarget);
		if (dist < 2 * size)
			return true;
		/*
		 * if (dist < (size * 5) && (index < maxIndex)) { GamaPoint newPt = currentTarget = targets.get(index + 1);
		 * IList<IShape> pts = GamaListFactory.create(Types.GEOMETRY); pts.add(currentTarget); pts.add(newPt); return
		 * location.euclidianDistanceTo(Spatial.Creation.line(scope, pts)) < dist; }
		 */
		return false;
